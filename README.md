...
- **"Select every Unique"/"Select every Set"**, in the **Select** popup next to Filters — turns on Unique (or Set) for every base in the whole catalog that can drop as one, in one click, without disturbing whatever else you've already selected for those bases. Deliberately separate from the Filter popup's similarly-named "Has Unique"/"Has Set" toggles, which only narrow what's shown, not what's watched.
